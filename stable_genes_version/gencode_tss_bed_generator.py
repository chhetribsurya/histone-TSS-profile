...
def create_matching_refseq_bed(refseq_tss_positions, tss_positions, output_dir, verbose=False):
    ...
    # Write matched RefSeq BED file
    output_bed_path = os.path.join(output_dir, "refseq_matched_to_gencode.bed")
    with open(output_bed_path, "w") as f:
        for pos in refseq_tss_positions:
            f.write("\t".join(map(str, pos)) + "\n")
    
    print(f"Created matched RefSeq BED file: {output_bed_path}")
    print(f"Contains {len(refseq_tss_positions)} TSS positions matched to Gencode genes")
    
    # Also create a mapping file showing the relationship
    ...
    print(f"Created RefSeq to Gencode mapping file: {mapping_file}")
    
    # Create separate group BED files for the RefSeq data
    # Group information is stored in the ENSG ID mapping
    group_tss_positions = defaultdict(list)
    
    # Build a mapping from ENSG ID to group
    for pos in refseq_tss_positions:
        ensg_id = pos[6]
        found_group = False
        
        # Try to find the group for this ENSG ID by finding the matching Gencode TSS position
        for orig_pos in tss_positions:
            orig_ensg = orig_pos[3].split('_')[0]
            if orig_ensg == ensg_id:
                group = orig_pos[6]
        ...
        # If we couldn't find a group (which is unexpected), use group 0 as fallback
        if not found_group and verbose:
    ...
    for group, positions in group_tss_positions.items():
        ...
        print(f"Created RefSeq matched group {group} BED file: {group_file} - {len(positions)} TSS positions")
